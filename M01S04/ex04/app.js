var i = 0;

// for (i = 0; i <= 3; i++) {
//   console.log(i);
// }

// Modifica exemplul astefl incat bucla sa numere de la 1 la 100
// for (i = 1; i <= 100; i++) {
//   console.log(i);
// }

// Modifiaca exempul astfel incat bucla sa numere de la 1 la 99(dar nu schimba numarul 100)
// for (i = 1; i < 100; i++) {
//   console.log(i);
// }

// folosind keywordul brak opreste bucla atunci cand numarul este egal cu 50(dar afiseazal)
// for (i = 1; i < 100; i++) {
//   console.log(i);

//   if (i === 50) {
//     break;
//   }
// }

// folosind keywordul continue afiseaza doar numerele pare intre 1 si 100(inclusiv 100)
// for (i = 1; i <= 100; i++) {
//   if (i % 2 !== 0) {
//     continue;
//   }

//   console.log(i);
// }

// in mod similar, afiseaza doar numerele care sunt divizibile cu 5
// for (i = 1; i <= 100; i++) {
//   if (i % 5 === 0) {
//     console.log(i);
//   }
// }

//Modifica exemplul astfel incat bucla sa numere de la 1 la 89.
// for (i = 1; i < 99; i++) {
//   console.log(i);
// }

//Modifica exemplul astfel incat bucla sa numere de la 1 la 88 .
// for (i = 1; i < 99; i++) {
//   console.log(i);
// }

// //Folosind keywordul break opreste bucla atunci cand numarul este egal cu 42.
// for (i = 1; i <= 100; i++) {
//   console.log(i);

//   if (i === 45) {
//     break;
//   }
// }

//Folosind keywordul continue, afiseaza doar numele impare intre 1 si 89.
// for (i =1; i < 89; i++) {
//   if (i % 2 !== 0 )
//   console.log(i)
// }

//In mod similar, afiseaza doar numerele care sunt divizibile cu 3.
for (i = 1; i < 89; i++) {
  if (i % 3 === 0) {
    console.log(i);
  }
}
